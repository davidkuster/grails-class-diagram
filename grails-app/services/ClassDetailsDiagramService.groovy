...
class ClassDiagramNonDomainService {
    ...
    byte[] createDiagram(ClassDetails classDetails, ClassDiagramPreferences prefs) {
        def dotBuilder = createDotDiagram(classDetails, prefs)
            ...
            if (! prefs.showPackages) {
                ...
                classDetails.getAllPackageNames().each { packageName ->
                    subgraph("cluster_"+packageName) {
                        ...
                        buildClassesPerPackage(dotBuilder, classDetails, packageName, prefs)
                    }
                }
            }
    ...
    private void buildGraphDefaults(dotBuilder, prefs) {
    ...
    private void buildClassesPerPackage(dotBuilder, classDetails, packageName, prefs) {
        def classDetailsForPackage = classDetails.getClassesToDiagramForPackage(packageName)
        classDetailsForPackage.each { detail ->
            if (detail instanceof DomainDetail
                || detail instanceof ControllerDetail
                ...
                buildConfigurableClass(dotBuilder, detail, 'showEnumAsProperty', prefs)
            }
        }
    }


    // used for Grails artefact classes
    private void buildArtefactClass(dotBuilder, detail, prefs) {
        dotBuilder."${detail.nodeName}" ([label:formatNodeLabel(detail, prefs)])
    }

    // used for classes that can be shown or hidden according to the user preferences
    private void buildConfigurableClass(dotBuilder, detail, showProp, prefs) {
        if (! prefs."$showProp") {
            dotBuilder."${detail.nodeName}" ([label:formatNodeLabel(detail, prefs)])
        }
    }
